...
from gzbuilder_analysis.data import get_sdss_cutout, download_json, download_image, get_montage_cutout, get_rotation_correction
...
try:
    ...
    print(
        'Could not find exports in local file system,',
        'requesting them from the Zooniverse'
    )
...
locations = subjects.pop('locations').apply(json.loads).apply(pd.Series).rename(columns={'0': 'difference', '1': 'image', '2': 'model'})
bands = ['r']
